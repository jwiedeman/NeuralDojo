...
* **2025-11-21 — Operations summary CLI:** Wrapped `compile_operations_summary` in `scripts/operations_summary.py`, exposing threshold overrides, JSON export, and regression coverage so operations teams can integrate guardrail alerts into automation flows without bespoke notebooks.
  - Notes (2025-11-28): Extended the summary pipeline and CLI with benchmark-aware diagnostics so operations reviews can track excess return, tracking error, information ratio, beta, and correlation without bespoke notebooks.
* **2025-11-22 — Guardrail enforcement release:** Landed the configurable `GuardrailPolicy`, service guardrail endpoint, and end-to-end regression coverage so trade logs can be clipped or reviewed against exposure/tail thresholds directly from automation clients.
* **2025-11-23 — Continuous retraining scheduler:** Delivered the file-change-aware `RetrainingScheduler` with cooldown controls so dataset refreshes automatically queue `run_retraining_plan` executions, pairing the automation orchestrator with a deployment-ready trigger path and regression coverage in `tests/test_retraining_scheduler.py`.
* **2025-11-24 — Guardrail exposure telemetry:** Extended guardrail enforcement to emit symbol/sector/factor exposure snapshots alongside metrics and violations. The FastAPI endpoint now surfaces these summaries, and documentation has been updated so operations teams can plug the data into dashboards without recomputing peak exposures.
* **2025-11-26 — Walk-forward evaluation CLI:** Introduced `scripts/walkforward_eval.py` to turn prediction parquet files into per-split walk-forward metrics and aggregated Sharpe/drawdown summaries. The CLI writes parquet + JSON artefacts for reporting automation and hooks into Phase 3 readiness gates without needing bespoke notebooks.
* **2025-11-27 — Service metrics fallback:** Hardened the FastAPI service so Prometheus instrumentation gracefully degrades when `prometheus_client` is unavailable, exposing a lightweight in-process registry that keeps `/metrics` responses and guardrail toggles operational in minimal environments.
* **2025-11-27 — Pretraining benchmark automation:** Extended the pretraining comparison CLI with delta aggregation and regression coverage so warm-start lift (validation loss, ROI, runtime) is quantified automatically after each benchmark run and the optimisation log can record trends without ad-hoc notebooks.
* **2025-12-02 — Omni vs. hybrid benchmark automation:** Ran the synthetic multi-universe backbone comparison via `scripts/benchmarks/backbone_comparison.py`, publishing results and Markdown summaries under `docs/benchmarks/backbone_comparison.*` so the optimisation log now anchors empirical comparisons ahead of GPU-scale reruns.
* **2025-11-27 — Test harness bootstrap:** Added a PyTest `conftest.py` shim that injects the project and repository roots onto `sys.path`, keeping the `market_nn_plus_ultra` package and top-level `scripts` module importable without an editable install. This unblocks local optimisation work that depends on the regression suite while containerised workflows are being prepared.
* **2025-11-27 — Analyst feedback tooling release:** Added the `market_nn_plus_ultra.reporting.annotations` module, SQLite validation hooks, and `scripts/annotate_trades.py` CLI so reviewers can capture decisions, rationales, and context windows with regression coverage and reporting integration hooks.
* **2025-11-25 — Live monitoring instrumentation:** Wired Prometheus metrics into the FastAPI service, exposed `/metrics` for scraping, and documented SLOs plus alert policies so operations can react to latency or guardrail degradations without sprint-level re-triage.
...
* **2025-11-20 — Regime attribution reporting:** Extended the reporting pipeline with regime-specific attribution tables, adding volatility/liquidity breakdowns, Markdown/HTML rendering, and regression coverage so automated reports now highlight which market states drive performance.
...
5. **Service interface** — Scaffold a FastAPI or gRPC inference service in `market_nn_plus_ultra.service` that mirrors the existing market agent contract, streams the richer Plus Ultra telemetry, and exposes curriculum configuration for reinforcement fine-tuning. **Status:** ✅ Completed — FastAPI service module plus CLI launcher now expose prediction, configuration, curriculum, and reload endpoints with telemetry-aware responses and regression coverage.
   ...
   - *Resilience update (2025-11-27):* Added a dependency-optional metrics backend so `/metrics` responses and guardrail toggles continue working in lean deployments where `prometheus_client` is absent, aligning service readiness with monitoring rollout plans.
6. **Walk-forward evaluation CLI** — Provide a first-class CLI under `scripts/walkforward_eval.py` that consumes prediction artefacts, emits per-split walk-forward metrics, and writes aggregated Sharpe/drawdown summaries for reporting automation. **Status:** ✅ Completed — the CLI now loads CSV/Parquet predictions, runs `WalkForwardBacktester`, exports metrics to Parquet/CSV, and records JSON summaries highlighting best Sharpe and worst drawdown splits for deployment reviews.
   - *Scoping notes (2025-11-24):* Documented requirements for Phase 3 readiness gates: per-split Sharpe/drawdown metrics, aggregated statistics suitable for dashboards, and JSON payloads alignable with operations summaries.
   - *Implementation (2025-11-26):* Implemented CLI wiring, summary aggregation (mean/median/std/min/max per metric), and extreme-split descriptors so monitoring pipelines can ingest outputs without bespoke notebooks. Added regression coverage in `tests/test_walkforward_eval_script.py` and README quickstart guidance.
...
2. **Live monitoring** — Integrate Prometheus metrics and alerting hooks into the inference service. Document SLOs and alert policies in `docs/operations.md`, including live decision support for extending or branching experiments. **Status:** ✅ Completed — Prometheus counters, histograms, and gauges now expose request health and guardrail telemetry via `/metrics`, and the new operations playbook captures SLOs plus alert thresholds for on-call teams.
...
3. **Risk guardrails** — Build guardrail modules that enforce exposure, turnover, tail-risk thresholds, and sector/factor caps during inference, configurable via YAML, and aligned with reinforcement fine-tuning outputs. **Status:** ✅ Completed — guardrail enforcement now ships via `GuardrailPolicy`, YAML-driven configuration, and a FastAPI endpoint for trade-log validation.
   ...
   - *Implementation landing (2025-11-22):* Added the `GuardrailPolicy` enforcement module with YAML-configurable thresholds, service endpoint wiring, and regression coverage validating exposure scaling, sector caps, and warn-only tail guardrails.
4. **Analyst feedback loop** — Add review tooling in `market_nn_plus_ultra.reporting` that lets human supervisors annotate trades, flag anomalies, and record veto rationales stored in SQLite for future offline RL. **Status:** ✅ Completed — shipped `market_nn_plus_ultra.reporting.annotations`, the `scripts/annotate_trades.py` CLI, SQLite schema/validation updates, and regression coverage so analysts can persist structured decisions that feed PPO replay buffers, reporting automation, and operations reviews.
   ...
   - *Landing notes (2025-11-27):* Delivered annotation schema + validation, CLI tooling, regression coverage, and documentation updates so analyst feedback can be captured reproducibly and reused by reporting, PPO replay buffers, and automation pipelines.
